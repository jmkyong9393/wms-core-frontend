...
- **4.1 역량 맞춤형 분업 전략 (7인 체제 - 실명 R&R 기반):**
  ...
  - **AI Lead (홍경표, 박준희):** LangGraph 다중 에이전트 파이프라인(Vision, Policy, Critic, Report) 조립 및 최적화, 대시보드용 AI 트렌드 리포트 프롬프트 튜닝.
  - **Data/MLOps (소한민, 고영빈):** 원시 이미지 데이터 정제, 오토 라벨링 파이프라인 구축, 소형 모델 지식 증류 리서치.
  - **QA/CI-CD (박민우):** 백엔드 단위/통합 테스트, GitHub Actions 기반 린팅(Linting) 및 자동 배포 파이프라인 구축.
  - **Frontend (고영빈, 박준희):** FE-1(모바일 카메라/Canvas 리사이징 전담), FE-2(PC 관리자 대시보드 UI 전담).

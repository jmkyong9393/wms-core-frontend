...
- **Orchestration & AI Worker Layer (API & Worker 분리):** 
  ...
  - **AI Worker 데몬:** Celery/Redis를 배제하고 LangGraph 기반 Supervisor (Star Topology) 상태 머신 구동. K8s 오토스케일링(HPA) 환경에서의 메모리 증발 및 중복 실행을 막기 위해 DB 큐를 Redis 브로커 및 Celery Task로 폴링.

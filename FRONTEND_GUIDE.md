...
### 🧑‍💻 고영빈 (UI/UX 및 컴포넌트 설계 주도)
주로 **사용자 눈에 보이는 뷰(View)와 레이아웃 구조**를 책임집니다.
- **주요 작업 폴더:** `app/` (페이지 라우팅), `components/` (UI 컴포넌트 덩어리)
- **주요 업무:** 
  - 기획 문서 및 요구사항을 바탕으로 공통 버튼, 인풋, 사이드바 등 재사용 가능한 UI 컴포넌트 제작
  - Tailwind CSS를 활용한 반응형 웹 퍼블리싱 및 UI/UX 구현

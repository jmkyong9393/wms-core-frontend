...
const BASE_CHIP_CLASS =
  'inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold transition-colors cursor-pointer';
...
export function DefectLayerToggle({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      >
        <span
          className={`h-2 w-2 rounded-sm ${showConfirmed ? 'bg-red-500' : 'bg-gray-300 dark:bg-zinc-600'}`}
        />
        확정 결함
      </button>
      <button
      ...
      >
        <span
          className={`h-2 w-2 rounded-sm ${showYolo ? 'bg-orange-500' : 'bg-gray-300 dark:bg-zinc-600'}`}
        />
        YOLO 후보
      </button>
    </div>

  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col gap-4">
      <div className="flex flex-col items-center text-center py-4">
        <PackageCheck className="h-14 w-14 text-green-600 mb-3" />
        <h1 className="text-xl font-bold text-gray-900">출고가 확정되었습니다</h1>
        <p className="text-sm text-gray-500 mt-1">주문 번호 {result.order_id}</p>
      </div>

      {label.is_demo && (
        <div className="flex items-center gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-gray-900">{label.title}</h2>
          <span className="text-xs font-semibold text-gray-500">{result.shipping_carrier}</span>
        </div>

        <div className="space-y-0.5">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">송장 번호</p>
          <p className="text-sm font-mono font-bold text-gray-800">{result.waybill_number}</p>
        </div>

        <div className="grid grid-cols-2 gap-3 text-xs text-gray-500">
          <div>
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">집화일</p>
        ...
        <div className="pt-2 border-t border-gray-100">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">송장 바코드</p>
